...
| מסד נתונים + אחסון | Supabase | פרויקט של rordan | **toptik2026's Org (Free)** | ✅ הועבר 2026-08-24 |
...
## 3. Supabase — ✅ הועבר (2026-08-24)
- **בוצע ניקוי אחסון לפני ההעברה:** נמחקו 9,565 קבצים יתומים (1.36GB) משאריות ייבואים
  חוזרים — אומת פעמיים שאף מוצר לא מפנה אליהם. אחרי הניקוי: **96MB אחסון / 28MB DB**
  → נכנס במגבלות Free (1GB/500MB).
- **Transfer project** בוצע מ-org של rordan (Pro) אל **toptik2026's Org (Free)** של הלקוח.
  ה-instance הוקטן ל-Nano עם ~דקה downtime. ה-URL וה-keys לא השתנו → אפס שינוי ב-Vercel.
- אומת אחרי ההעברה: אתר 200 · 24 מוצרים ב-API · תמונות 200 · אדמין תקין.
- שים לב ב-Free: אין גיבויים יומיים; פרויקט מושהה אחרי שבוע ללא פעילות (ה-cron היומי
  של האתר שומר על פעילות).
- **`toptik-whatsapp-agent-prod` (הסוכן וואטסאפ) — ✅ הועבר גם הוא (2026-08-24):**
  הפרויקט הריק "toptik2026's Project" נמחק (אומת ריק — 0 טבלאות) כדי לפנות את הסלוט
  השני במגבלת ה-Free, תפקיד rordan שודרג ל-Owner ב-org של הלקוח, והסוכן (12MB DB,
  0 אחסון) הועבר ל-**toptik2026's Org**. ה-keys לא השתנו → השירות ב-agent.toptik.co.il
  ממשיך לעבוד ללא שינוי תצורה.
- **מצב סופי:** שני הפרויקטים של טופטיק (אתר + סוכן) תחת org הלקוח (Free, 2/2 סלוטים).
  ב-org של rordan לא נשאר שום רכיב של טופטיק.

### הפרוצדורה המקורית (בוצעה; נשמר לתיעוד)
